<template>
  <div class="favorite-page" @keydown="onKeyDown" tabindex="0" ref="pageRef">
    <div class="page-header">
      ...
      <h2>收藏夹</h2>
      <span class="count-badge">共 {{ favoriteItems.length }} 题</span>
    </div>

    <el-empty v-if="!favoriteItems.length" description="暂无收藏题目" />
    ...
    <template v-else>
      ...
      <div class="question-list">
        ...
        >
          ...
          <div class="item-info">
            <span class="item-type">{{ item.typeName || '题目' }}</span>
            <p class="item-content">{{ truncate(item.content || item.material) }}</p>
          </div>
          <button class="unfavorite-inline-btn" @click.stop="removeFavorite(item.id)">取消收藏</button>
        </div>
      </div>
        ...
        <div class="drawer">
          <div class="drawer-header">
            ...
            <div class="drawer-nav">
              ...
              <span class="nav-pos">{{ selectedIdx + 1 }} / {{ favoriteItems.length }}</span>
              <button class="nav-arrow" :disabled="selectedIdx >= favoriteItems.length - 1" @click="selectItem(selectedIdx + 1)">
            ...
            <button class="unfavorite-btn" @click="removeFavorite(selectedItem?.id)">取消收藏</button>
          </div>

          <div class="drawer-body">
            <div class="detail-meta">
              <span class="type-tag" :style="{ background: typeColor(selectedItem?.type) }">
                {{ selectedItem?.typeName }}
              </span>
              <span class="favorite-badge">已收藏</span>
            </div>

            <div class="question-content" v-html="getContent(selectedItem)"></div>

            <div class="answer-section">
              <!-- 单选/多选 -->
              <div v-if="['single','multiple'].includes(selectedItem?.type)" class="options-list">
                <div v-for="opt in selectedItem.options" :key="opt.label" class="option-item" :class="{ 'is-correct': isCorrectOpt(selectedItem, opt.label) }">
                  <span class="opt-label">{{ opt.label }}</span>
                  <span class="opt-text">{{ opt.text }}</span>
                  <span v-if="isCorrectOpt(selectedItem, opt.label)" class="correct-mark">✓</span>
                </div>
              </div>
              <!-- 判断 -->
              <div v-else-if="selectedItem?.type === 'judge'" class="simple-answer">
                <span class="answer-label">正确答案：</span>
                <span class="answer-value">{{ formatJudge(selectedItem.correctAnswer) }}</span>
              </div>
              <!-- 填空 -->
              <div v-else-if="selectedItem?.type === 'blank'" class="simple-answer">
                <span class="answer-label">正确答案：</span>
                <span class="answer-value">{{ formatBlank(selectedItem.correctAnswer) }}</span>
              </div>
              <!-- 简答 -->
              <div v-else-if="selectedItem?.type === 'essay'" class="essay-answer">
                <div class="answer-block-title">参考答案</div>
                <div class="answer-block-content">{{ selectedItem.correctAnswer }}</div>
              </div>
              <!-- 完形填空 -->
              <div v-else-if="selectedItem?.type === 'cloze'" class="cloze-section">
                <div v-for="(blank, bi) in selectedItem.blanks" :key="blank.id" class="cloze-item">
                  <div class="cloze-item-title">第{{ bi + 1 }}空</div>
                  <div class="options-list">
                    <div v-for="opt in blank.options" :key="opt.label" class="option-item" :class="{ 'is-correct': selectedItem.correctAnswer[blank.id] === opt.label }">
                      <span class="opt-label">{{ opt.label }}</span>
                      <span class="opt-text">{{ opt.text }}</span>
                      <span v-if="selectedItem.correctAnswer[blank.id] === opt.label" class="correct-mark">✓</span>
                    </div>
                  </div>
                </div>
              </div>
              <!-- 复合题 -->
              <div v-else-if="selectedItem?.type === 'composite'" class="composite-section">
                <div v-for="(sq, si) in selectedItem.subQuestions" :key="sq.id" class="sub-item">
            ...
            <div v-if="selectedItem?.explanation && !['composite'].includes(selectedItem?.type)" class="explanation">
              <span class="exp-label">解析：</span>
              <span class="exp-text">{{ selectedItem.explanation }}</span>
            </div>
          </div>
        </div>
      </div>
...
const favoriteItems = computed(() => {
  return store.favorites.map(qId => {
    // 优先从 favoriteBook 取，兼容旧数据从 wrongBook 取
    return store.favoriteBook[qId] || store.wrongBook[qId]?.question || null
  }).filter(Boolean)
})

const selectedItem = computed(() => favoriteItems.value[selectedIdx.value] || null)
const pageOffset = computed(() => (currentPage.value - 1) * pageSize)
const pagedItems = computed(() => favoriteItems.value.slice(pageOffset.value, pageOffset.value + pageSize))
...
function onKeyDown(e) {
  ...
  if (e.key === 'ArrowDown' || e.key === 'ArrowRight') {
    e.preventDefault()
    if (selectedIdx.value < favoriteItems.value.length - 1) selectItem(selectedIdx.value + 1)
...
async function removeFavorite(qId) {
  if (!qId) return
  try {
    await ElMessageBox.confirm('确认取消收藏此题吗？', '取消收藏', {
      confirmButtonText: '确认', cancelButtonText: '取消', type: 'warning'
    })
    store.toggleFavorite(qId)
    ElMessage.success('已取消收藏')
    closeDrawer()
    if (selectedIdx.value >= favoriteItems.value.length) {
      selectedIdx.value = Math.max(0, favoriteItems.value.length - 1)
    }
  } catch { /* 取消 */ }
}
...
<style scoped>
.favorite-page { max-width: 800px; margin: 0 auto; outline: none; display: flex; flex-direction: column; height: calc(100vh - 64px - 48px); }
...
.count-badge { padding: 3px 10px; border-radius: 10px; font-size: 13px; background: rgba(255,149,0,0.1); color: #FF9500; font-weight: 500; }
...
.list-item:hover { border-color: #FF9500; }
.list-item.active { border-color: #FF9500; background: rgba(255,149,0,0.04); }
...
.unfavorite-inline-btn { flex-shrink: 0; padding: 3px 10px; border: 1px solid #e4e7ed; border-radius: 6px; background: none; font-size: 12px; color: #909399; cursor: pointer; transition: all 0.15s; white-space: nowrap; }
.unfavorite-inline-btn:hover { border-color: #FF9500; color: #FF9500; }
...
.nav-arrow:hover:not(:disabled) { border-color: #FF9500; color: #FF9500; }
.nav-arrow:disabled { opacity: 0.35; cursor: not-allowed; }
.nav-pos { font-size: 13px; color: #606266; min-width: 60px; text-align: center; }
.unfavorite-btn { padding: 5px 12px; border: 1.5px solid #e4e7ed; border-radius: 8px; background: none; font-size: 13px; color: #606266; cursor: pointer; transition: all 0.15s; flex-shrink: 0; }
.unfavorite-btn:hover { border-color: #FF9500; color: #FF9500; }
...
.favorite-badge { padding: 3px 10px; border-radius: 8px; font-size: 12px; background: rgba(255,149,0,0.1); color: #FF9500; font-weight: 500; }
